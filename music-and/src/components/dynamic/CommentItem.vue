<template>
  <div class="mt-4">
    <div class="flex items-center">
      <img :src="comment.userAvatar" alt="User Avatar" class="w-10 h-10 mr-3 rounded-full">
      <div class="flex flex-col">
        <span class="font-bold text-blue-500">{{ comment.userName }}</span>
        <span class="text-sm text-gray-500">{{ getTimeAgo(comment.createTime) }}</span>
      </div>
    </div>
    ...
    <div class="flex items-center mt-2">
      ...
      <button @click="toggleSubComments" class="flex items-center text-gray-500 hover:text-blue-500 ml-10">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
    ...
    <!-- 子评论输入框 -->
    <div v-if="comment.showComments" class="mt-2 ml-16">
      <div class="flex items-center">
        <img :src="comment.userAvatar" alt="User Avatar" class="w-10 h-10 mr-3 rounded-full">
        <input v-model="comment.newComment" @keyup.enter="addSubComment" type="text" placeholder="添加评论..." class="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>
    </div>

    <!-- 子评论列表 -->
    <div v-if="comment.comments.length > 0 && comment.showComments" class="mt-2 ml-16">
